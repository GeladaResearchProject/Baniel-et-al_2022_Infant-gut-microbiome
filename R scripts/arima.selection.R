arima.selection <- function(adj.data=NULL,
  ...
  metadata$age<-metadata$AgeMonth
  metadata$sampid<-metadata$SampleID
  ...
  age.sexsel <- age[names(sex)] # gets the ages of only the males (selected sex)
  age.sexsel.sorted <- age.sexsel[order(age.sexsel)] # orders by age
  adj.data.sexsel <- adj.data[,names(sex)] # gets GE of males 
  ...
  selpeaks.sexsel.unique <- do.call(rbind,lapply(setNames(as.list(seq_len(nrow(age.counts))),age.counts$age.sexsel),function(n) {
    ...
    return(estage)})) # mean counts by unique age?? 
  ...
  #plot.ts(ts.sexsel[[2]],type="b") # Visualization of a single TS
  ...
  gc()
  predicted.sexsel.nonzero <- t(as.data.frame(mclapply(loess.sexsel.nonzero,predict,evalage,mc.cores = n.cores),row.names = evalage,check.names=FALSE))
  gc()
